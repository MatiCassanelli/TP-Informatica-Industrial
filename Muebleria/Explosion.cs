...
namespace Muebleria
{
    public partial class Explosion : Form
    {
        ...
        private List<padre_componente_publicado> ObtenerComponentes(string prod)
        {
            ...
            var componentes = db.padre_componente_publicado.SqlQuery("SELECT pcp.* FROM `padre-componente-publicado` pcp INNER JOIN(SELECT distinct pcp2.idPadreP, pcp2.fecha_aplicacion FROM `padre-componente-publicado` pcp2 WHERE pcp2.idPadreP = " + P[0].ToString()+ " and pcp2.activado = 1 and pcp2.fecha_aplicacion <= " + fecha.ToString() + ") as t on pcp.idPadreP = t.idPadreP WHERE pcp.fecha_aplicacion = t.fecha_aplicacion and pcp.activado = 1 and pcp.version >= all(SELECT distinct pcp4.version FROM `padre-componente-publicado` pcp4 WHERE pcp4.idPadreP = " + P[0].ToString() + " and pcp4.activado = 1 and pcp4.fecha_aplicacion <= " + fecha.ToString() + ");");
